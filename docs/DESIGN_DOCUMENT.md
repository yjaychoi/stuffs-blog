...
This section defines implementation guidance anchored to `docs/mockup.html`.
...
1. `docs/mockup.html` defines the minimum acceptable visual hierarchy and polish for post detail pages.
...
1. The visual direction is Monokai-inspired, but neutral grays are allowed for borders, muted text, and surfaces as used in `docs/mockup.html`.
...
2. For the canonical post-detail route (the `docs/mockup.html` benchmark), the spec must define:
...
5. Visual determinism is achieved by mapping route fixtures directly to `docs/mockup.html`; no additional screenshot-style interpretation layers are required.
...
2. Composition and chrome decisions in this section must map directly to `docs/mockup.html` as source of truth.
